...
export default function ChatNavigation({ psikologInfo, consultationTime, notes, chat_status }) {
    ...
    const { name = "", photo_profile = "", is_online = false } = psikologInfo || {};
    ...
    if (!psikologInfo || !consultationTime) {
        return <div className="bg-primarylight2 py-5 px-6"><Loading/></div>;
    }
                ...
                <div className="flex gap-4">
                    <button 
                         ...
                         className={`flex items-center text-whitebg text-m font-semibold gap-2 px-4 rounded-lg
                             ${chat_status === 'completed' ? 'bg-primary cursor-pointer' : 'bg-disable cursor-not-allowed'}
                         `}
                    >
                        <Image
                    ...
                    <div
                        className={`rounded-lg text-s font-semibold flex items-center px-4 
                            ${chat_status === 'completed' ? 'bg-gray-400 text-white' : 'bg-primary text-white'}
                            ${chat_status === 'completed' ? 'grayscale(100%) opacity-50' : ''}`}
                    >
                        <p className={`${chat_status === 'completed' ? 'opacity-100' : ''}`}>
                            {chat_status === "completed" ? "Selesai" : remainingTime ?? "Belum dimulai"}
                        </p>
                    </div>
                </div>
            </div>
