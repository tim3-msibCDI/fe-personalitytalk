...
export default function FormPilihPsikolog({ onSelectPsikolog }) {
  ...
  useEffect(() => {
    const fetchDates = async () => {
      try {
        ...
        if (!selectedTopic) {
          console.error(
            "Topik tidak tersedia di localStorage atau tidak valid:",
            selectedTopic
          );
          return;
        }
        ...
        if (data && data.data) {
          const formattedDates = Object.entries(data.data).map(
            ([date, professionals]) => ({
              dayMonth: date,
  ...
  return (
    <div className="flex justify-between gap-8 py-6">
      <div className="w-2/5">
        <h1 className="text-h3 font-semibold text-textcolor mb-2">
          Jenis Profesional
        </h1>
        <p className="text-s mb-6">
          Terdapat 2 Jenis profesional yang disediakan oleh PersonalityTalk
        </p>
        ...
        <div className="bg-primarylight rounded-lg p-4 mb-6">
          ...
          <ul className="list-disc list-inside text-s">
            <li>
              Lulusan <b>S1 Psikologi</b> atau <b>Bimbingan Konseling</b>
            </li>
            <li>
              Memiliki <b>Sertifikat Pelatihan</b>
            </li>
            <li>
              Pendekatan terapi berbasis <b>konseling psikologi</b>
            </li>
            <li>
              Harga mulai dari <b>Rp.50.000</b>
            </li>
          </ul>
        </div>
        ...
        <div className="bg-primarylight rounded-lg p-4 mb-6">
          ...
          <ul className="list-disc list-inside text-s">
            <li>
              Lulusan <b>Profesi Psikologi</b>
            </li>
            <li>
              Memiliki <b>Surat Ijin Praktik Psikologi</b>
            </li>
            <li>
              Mampu melakukan <b>Asesmen Psikologi</b>
            </li>
            <li>
              Pendekatan terapi dengan beragam <b>teknik psikoterapi</b>
            </li>
            <li>
              Harga mulai dari <b>Rp.100.000</b>
            </li>
          </ul>
        </div>
      </div>
            ...
            <form className="w-full">
              <div className="relative">
                ...
                <input
                  type="search"
                  id="default-search"
                  className="block w-full p-4 ps-10 text-sm text-text2 border border-text2 rounded-lg bg-whitebg focus:ring-primary focus:border-primary"
          ...
          {filteredData.length === 0 ? (
            ...
            <div className="mt-6 max-h-[400px] overflow-y-auto pr-4">
              {filteredData.map((item, index) => (
                <div
                  key={index}
                  className="flex gap-3 mb-4 border-b border-textcolor pb-4"
                >
                  <div className="w-24 h-24 rounded overflow-hidden">
                    <Image
                      src={`${process.env.NEXT_PUBLIC_IMG_URL}/${item.photo_profile}`}
                      alt={`Foto ${item.name}`}
                      width={100}
                      height={100}
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <div className="flex items-center">
                        <Image
                          ...
                          height={18}
                        />
                        <p className="ml-1 text-s">
                          {item.years_of_experience} tahun
                        </p>
                      </div>
                      <span className="text-gray-400">|</span>
                      <div className="flex items-center">
