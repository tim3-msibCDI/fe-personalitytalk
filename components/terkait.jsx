  ...
  return (
    <div className="mr-4 lg:mr-8 ml-4 lg:ml-8 px-14 mb-10">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-h2 text-textcolor font-semibold">
          Psikolog Terkait
        </h2>
        ...
        {psikologTampil.map((item, index) => (
          <div key={index} className="flex gap-3 w-[45%]">
            ...
            <div className="flex flex-col gap-4">
              {" "}
              {/* Menambah jarak antar elemen di konten kanan */}
              {/* Baris 1: Nama Psikolog dan Jumlah Jadwal */}
              <div className="flex justify-between">
                <p className="mr-10">{item.name}</p>{" "}
                {/* Menambah margin-right agar lebih jauh dari jadwal */}
              ...
              <div className="flex justify-between items-center mt-4 gap-4">
                {" "}
                {/* Menambah margin-top agar topik lebih jauh dari button */}
                <p>
                  {item.topik.slice(0, 2).join(", ")}{" "}
                  {/* Menampilkan dua topik awal */}
                  {item.topik.length > 2 && (
                    <span className="ml-2">+{item.topik.length - 2}</span>
                  )}{" "}
                  {/* Menampilkan jumlah sisa topik */}
                </p>
                <button className="bg-primary hover:bg-primary-dark text-white py-2 px-4 rounded ml-10">
                  {" "}
                  {/* Menambah margin-left pada button */}
                  Pilih psikolog
                </button>
              </div>
            </div>
