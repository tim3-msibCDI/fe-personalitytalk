    ...
    const fetchReview = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/consultation/detail-review`,
          {
            headers: {
        ...
        if (error.response && error.response.status === 404) {
          console.warn(
            "Review tidak ditemukan, pengguna bisa membuat review baru."
          );
        } else {
  ...
  const handleRatingSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/consultation/submit-review`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <Image
                ...
                src={
                  star <= rating
                    ? "/image/icons/star-filled.png"
                    : "/image/icons/star-outline.png"
                }
                alt={`Star ${star}`}
                width={24}
                height={24}
