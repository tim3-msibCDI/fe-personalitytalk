...
export default function Catatan({ onClose, consulId }) {
  ...
  const idTransaction =
    consulId ||
    JSON.parse(localStorage.getItem("transactionData"))?.id_transaction;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/consultation/submit-complaint`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <div className="bg-primary text-whitebg px-6 py-4 rounded-t-lg flex justify-between items-center">
        <div className="flex flex-col">
          ...
          <p className="text-s font-light">
            Berikan catatan keluhan kepada psikolog untuk dibaca sebelum memulai
            Konsultasi
          </p>
        </div>
        <Image
