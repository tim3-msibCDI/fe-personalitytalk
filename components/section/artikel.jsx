...
export default function Artikel() {
    ...
    useEffect(() => {
        ...
        const fetchArticles = async () => {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/landing-page/articles/recommendation`, {
                    method: "GET",
                    headers: {
                ...
                {articles.map((item) => (
                    ...
                    >
                        <div className="bg-primarylight2 rounded-lg shadow-md p-4 cursor-pointer">
                            ...
                            <div className="py-4">
                                <h5 className="text-m font-bold text-textcolor">{item.article_title || "Judul Artikel"}</h5>
                                <p className="text-s text-textcolor">{item.publication_date || "Tanggal tidak tersedia"}</p>
                            </div>
                        </div>
                    </Link>
                ))}
