            ...
            <div className="w-full">
                {layanan.map((item, index) => (
                    <div key={item.id} id={item.slug} className="scroll-mt-28">
                        ...
                        <Link href={`/${item.slug}`}>
                            <div className={`flex flex-row ${item.id % 2 === 0 ? 'flex-row-reverse' : ''} justify-between items-center p-4 gap-16 p-10 cursor-pointer`}>
                                <div className="w-1/3">
                                    <Image
                                        src={item.images}
                                        alt={item.imagealt}
                                ...
                                <div className={`${item.id % 2 === 0 ? 'text-right' : 'text-left'} w-2/3 ${item.id % 2 === 0 ? 'pr-4' : 'pl-4'}`}>
                                    <div className={`flex items-center mb-2 ${item.id % 2 === 0 ? 'justify-end' : ''}`}>
                                        <Image
