...
// Fungsi untuk menambahkan harga Psikolog
export async function addPsikologPrice(data) {
  try {
    const response = await fetch(`${API_URL}/admin/psikolog-price`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${getToken()}`,
    ...
    if (!response.ok) {
      const errorData = await response.text();
      throw new Error(errorData || "Gagal menambahkan harga Psikolog");
    }

    return { success: true, message: "Harga Psikolog berhasil ditambahkan" };
  } catch (error) {
    console.error("Error in addPsikologPrice:", error.message);
    return { success: false, message: error.message };
  }
}

// Fungsi untuk mengupdate data harga psikolog
export async function updatePsikologPrice(id, data) {
  try {
    const response = await fetch(`${API_URL}/admin/psikolog-price/${id}`, {
      method: "PUT",
      headers: {
    ...
    if (!response.ok) {
      const errorData = await response.text();
      throw new Error(errorData || "Gagal mengedit harga Psikolog");
    }

    return { success: true, message: "Harga Psikolog berhasil diperbarui" };
  } catch (error) {
    console.error("Error in updatePsikologPrice:", error.message);
    return { success: false, message: error.message };
  }
}

// Fungsi untuk mengambil data psikolog harga
export async function getPsikologPriceById(id) {
  try {
    const response = await fetch(`${API_URL}/admin/psikolog-price/${id}`, {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      const errorData = await response.text();
      throw new Error(errorData || "Gagal mengambil data Psikolog");
    }

    const data = await response.json();
    return { success: true, data };
  } catch (error) {
    console.error("Error in getPsikologPriceById:", error.message);
    return { success: false, message: error.message };
  }
}
